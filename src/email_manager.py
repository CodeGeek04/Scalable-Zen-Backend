...
# Integrated functionalities from Zenpal's calendar_api.py
...
def fetch_unread_threads(gmail_service):
    "fetches unread threads from gmail and returns them in a list"

    # Call the Gmail API
    results = gmail_service.users().threads().list(userId='me', labelIds=['INBOX'], q="is:unread").execute()
    ...
    if not threads:
        ...
        for thread in threads:
            thrd = service.users().threads().get(userId='me', id=thread['id']).execute()
