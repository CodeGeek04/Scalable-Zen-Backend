...
from list_emails import get_all_emails, get_owner_email
...
def index():
    ...
    print("TOTAL NEW THREADS: ", len(new_threads))
    ...
    for thread in new_threads:
        ...
        alt_email, client_email = get_emails_from_thread(messages, assistant_email)

        if not alt_email or not client_email:
            print("THREAD NOT REGISTERED")
            print(mark_thread_as_read(gmail_service, thread['id']))
            continue

        if len(messages) == 1:
            continue
        
        print("ALTERNATE EMAIL: {}".format(alt_email))
        owner_email = get_owner_email(alt_email)
        ...
        # decoded_message = get_message_text(last_message)

        try:
            free_time = fetch_free_time(owner_email)
            print("FREE TIME: {}".format(free_time))
            body_reply, subject_reply = generate_resp(concatenated_messages, client_email, alt_email, free_time, assistant_email)
        ...
        mark_thread_as_read(gmail_service, thread['id'])
        reply_to_email_thread(gmail_service, latest_message, body_reply, client_email, thread['id'], owner_email)
    ...
    for email in new_emails:
        ...
        alt_email, client_email = get_emails(msg, assistant_email)

        if not alt_email or not client_email:
            print("EMAIL NOT REGISTERED")
            print(mark_email_as_read(gmail_service, email['id']))
            continue
        ...
        try:
            free_time = fetch_free_time(owner_email)
            print("FREE TIME: {}".format(free_time))
            body_reply, subject_reply = generate_resp(decoded_message, client_email, alt_email, free_time, assistant_email)
        except Exception as e:
            print("EXCEPTION: ", e)
    ...
    return "Check your server console.", 200
