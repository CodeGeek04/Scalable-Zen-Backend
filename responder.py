...
def generate_resp(history, sender_email, owner_email, free_time, assistant_email):
    ...
    body_prompt = '''Today is {}. You are working under, {} (the owner), who has free time slots: {}. 
                    They are trying to schedule a meeting with the client, {} (extract the name properly to write in email). 
                    Look at this conversation conversation: "{}". In case of multiple messages, odd numbered ones were sent by you and even by the client, so try to inculcate their preference while scheduling. Now, based on this conversation, you have to do one of the following:
                    1) Suggest free times for the meeting from available time slots that the client can choose from. Limit the duration to 30 minutes, and provide start + end time both. Be biased towards suggesting times in the next 2-3 days.
                    ...
                    Decide just on thing or the other and then frame the body of the email.
